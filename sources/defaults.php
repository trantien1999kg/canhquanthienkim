<?php
	...
	$bg_duan=$db->rawQueryOne("select photo_$lang as photo from #_bannerqc where hienthi=1 and type=? limit 0,1",array('bg_duan'));
	...
	$bg_quytrinh =$db->rawQueryOne("select photo_$lang as photo from #_bannerqc where hienthi=1 and type=? limit 0,1",array('bg_quytrinh'));
	...
	$projects=$db->rawQuery("select id , ten_$lang as ten , tenkhongdau_$lang as tenkhongdau , mota_$lang as mota , type , photo from #_baiviet where type=? ",array('du-an'));

	$quytrinh=$db->rawQuery("select id , ten_$lang as ten , tenkhongdau_$lang as tenkhongdau , mota_$lang as mota , type , photo from #_baiviet where type=? ",array('quy-trinh'));

	$service_c1=$db->rawQuery("select id, ten_$lang as ten, photo ,tenkhongdau_$lang as tenkhongdau,type from #_baiviet_list where hienthi=1 and noibat=1 and type=? order by stt asc",array('dich-vu'));

	$danhmucnoibat=$db->rawQueryOne("select id , title_$lang as title , mota_$lang as mota , type , photo from #_seopage where type=? limit 0,1",array('dich-vu'));
